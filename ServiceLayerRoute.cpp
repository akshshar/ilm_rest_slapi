                     ...
                     uint32_t label)
{
    ...
    routev4Ptr->set_prefixlen(prefixLen);
    routev4Ptr->mutable_routecommon()->set_admindistance(adminDistance);
    routev4Ptr->mutable_routecommon()->set_locallabel(label);
}
                         ...
                         uint32_t pathId)
{
    ...
    routev4PathPtr->mutable_nexthopaddress()->set_v4address(nextHopAddress);
    routev4PathPtr->mutable_nexthopinterface()->set_name(nextHopIf);
    if (pathtype == "protected" || pathtype == "backup") {
        routev4PathPtr->set_pathid(pathId);
        if (pathtype == "backup") {
            routev4PathPtr->add_protectedpathbitmap(protectedBitMap);
        }
    }
    routev4PathPtr->set_pathid(pathId);
    for (auto & label : labelstack) {
        LOG(INFO) << label;
        routev4PathPtr->add_labelstack(label);
    }
}
